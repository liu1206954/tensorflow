...
# 读取数据
conf = Config()
'''[ [prefix的one-hot, title的one-hot, label ],  [...]]'''
...
'''三个全连接层
对于每一层全连接层，除了神经元不一样，其他都一样，所以可以写一个函数复用。 $$ l_n = W_n x + b_1 $$
'''
def add_layer(inputs, in_size, out_size, activation_function=None):
    wlimit = np.sqrt(6.0 / (in_size + out_size))    #开根号
...
def batch_normalization(x, phase_train, out_size):
    ...
    with tf.variable_scope('bn'):
        ...
        '''每训练一个batch就用滑动平均值来更新整个训练集的平均值与方差'''
        ema = tf.train.ExponentialMovingAverage(decay=0.5)

        '''tf.control_dependencies 与 tf.identity 一起使用，进行流控制。
        即只有ema_apply_op 运行完才能返回更新后的滑动平均值。
        此处必须用 tf.identity， 否则 流控制无效，
        即 batch_mean 与 batch_var 不依赖 ema_apply_op，返回的便不是更新后的值。'''
        def mean_var_with_update():
            ema_apply_op = ema.apply([batch_mean, batch_var])
            with tf.control_dependencies([ema_apply_op]):
                return tf.identity(batch_mean), tf.identity(batch_var)

        '''若是在训练中，mean 与 var 是每次batch更新后的值，
        若不在训练中，则用最后更新完的值亦即整个训练集的平均值与方差。
...
with tf.name_scope('input'):
    ...
    query_batch = tf.placeholder(tf.float32, shape=[None, None], name='query_batch')    #train_data中第一列，prefix
    doc_batch = tf.placeholder(tf.float32, shape=[None, None], name='doc_batch')        #train_data中第二列， title
    doc_label_batch = tf.placeholder(tf.float32, shape=[None], name='doc_label_batch')  #train_data中第三列， label
    on_train = tf.placeholder(tf.bool)  #是否为训练
    keep_prob = tf.placeholder(tf.float32, name='drop_out_prob')    #keep_prob为控制dropoout比率的一个placeholder
...
with tf.name_scope('FC1'):
    ...
    query_l1 = add_layer(query_batch, conf.nwords, L1_N, activation_function=None)  #L1_N: 400
...
with tf.name_scope('FC2'):
    query_l2 = add_layer(query_l1, L1_N, L2_N, activation_function=None)    #L1_N: 400，L2_N：120
...
'''data_set:数据集
分别将data_set中第一列，第二列，第三列分别取出来组成query_in，doc_in，label
'''
def feed_dict(on_training, data_set, batch_id, drop_prob):
    query_in, doc_in, label = pull_batch(data_set, batch_id)    #分别将data_
